...
played_vs_optimal <- joined_human_ai %>% 
  ...
  select(Game, turn, p1_name, this_card, this_card_played, optimal_prob, gamestate_str)

played_vs_optimal %>% head()
```




Absolute mistakes (optimal probability 0)

```{r}
played_vs_optimal %>% 
  filter(optimal_prob == 0, this_card_played == 1)
```


```{r}
played_vs_optimal %>% 
  ggplot(aes(x = optimal_prob, y = this_card_played)) +
...
```{r}
# Errors to review pictures from

played_vs_optimal %>% 
  filter(is.na(optimal_prob), this_card_played == 1)
```