...
card_avg_by_turn %>% 
  ...
  #geom_text(aes(label = card), position = position_dodge(width=0.5)) +
  geom_line(size = 2, alpha = 0.75) +
  geom_point(size = 3, alpha = 0.75) +
  geom_abline(intercept = 1, slope = 0, color = "black") +
  labs(x = "Turn",
       y = "",
       title = "Relative Optimal Probabilities By Turn",
       legend = "Card")
...
card_avg_by_turn %>% 
  ...
  labs(x = "Turn",
       y = "",
       title = "Avgerage Optimal Probability (Normalized)") +
...
value_changes <- joined_human_ai %>% 
  ...
  arrange(turn) %>% 
   # next_turn_value is the value of the next turn. If game is over, use game outcome
