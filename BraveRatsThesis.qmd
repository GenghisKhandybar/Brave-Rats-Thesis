...
response_turns <- allTurns %>% 
  filter(s1_type == "s1r") %>% 
  pivot_longer(cols = c("p1_o0","p1_o1","p1_o2","p1_o3","p1_o4","p1_o5","p1_o6","p1_o7"), names_to = "p2_card_first", values_to = "optimal_responses") %>% 
  mutate(p2_card_first = as.numeric(substring(p2_card_first,5,6))) %>% 
  filter(optimal_responses!="")
...
There appears to be a small, but statistically insignificant difference inbetween the value changes from plays that are "miastakes" vs. non-mistakes.
